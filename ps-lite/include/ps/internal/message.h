...
enum DataType {
  ...
  FLOAT, DOUBLE, SERVER_MATRIX_META,REQ_MATRIX_META ,OTHER
};
...
DataType GetDataType() {
  if (SameType<V, int8_t>()) {
    ...
    return DOUBLE;
  } else if(SameType<V,ServerMatrixMeta>()){

     return SERVER_MATRIX_META;

  } else if(SameType<V,ReqMatrixMeta>()){
     
        return REQ_MATRIX_META;
